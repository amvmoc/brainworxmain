...
interface NeuralImprintPatternsHistogramProps {
  scores: NeuralImprintPatternScore[];
}

export function NeuralImprintPatternsHistogram({ scores }: NeuralImprintPatternsHistogramProps) {
  const maxScore = 100;
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 relative pb-8">
            ...
            <div className="relative h-full flex items-end justify-around gap-1 px-4">
              {scores.map((pattern) => {
                const heightPercent = (pattern.score / 100) * 100;
                return (
                  <div key={pattern.code} className="flex flex-col items-center flex-1 max-w-[50px]">
                    <div
                      className="w-full rounded-t transition-all"
                      style={{
                        height: `${heightPercent * 2}px`,
                        backgroundColor: getBarColor(pattern.severity),
                      }}
                    ></div>
      ...
      <div className="mt-8 grid md:grid-cols-2 gap-6">
        <div className="bg-gray-50 rounded-lg p-6 border border-gray-300">
          ...
          <div className="grid grid-cols-2 gap-3">
            {scores.map((pattern) => (
              <div key={pattern.code} className="flex items-center gap-2">
                <div
                  className="w-3 h-3 rounded-full border border-gray-600"
                  style={{ backgroundColor: getBarColor(pattern.severity) }}
                ></div>
                <span className="text-sm font-medium">{pattern.code}</span>
