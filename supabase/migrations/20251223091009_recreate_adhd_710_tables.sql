/*
  # Recreate ADHD 7-10 Assessment Tables

  This migration recreates the tables needed for the ADHD 7-10 Parent/Caregiver assessment.
  The ADHD 7-10 assessment requires both parent AND teacher/caregiver input.

  ## New Tables
  
  ### `adhd_assessments`
  Main assessment records for children aged 7-10
  - `id` (uuid, primary key)
  - `child_name` (text) - Name of the child being assessed
  - `child_age` (integer) - Age of the child (7-10)
...
CREATE TABLE IF NOT EXISTS adhd_assessments (
  ...
  child_age integer NOT NULL CHECK (child_age >= 7 AND child_age <= 10),
